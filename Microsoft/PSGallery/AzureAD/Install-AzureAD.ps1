...
#region Install AzureAD module
if (-not (Get-InstalledModule AzureAD -AllVersions -ErrorAction SilentlyContinue)) {
    Install-Module -Name AzureAD -Force -Confirm:$false -AllowClobber -Scope AllUsers
}
#endregion

#region Create Scheduled Task for updates
$ActionParams = @{
    Id = 'Update AzureAD PS Module'
    Execute = 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe'
    Argument = '-Command "Update-Module -Name AzureAD"'
}
...
$TaskParams = @{
    ...
    Description = 'Update AzureAD Module'
}
...
Register-ScheduledTask -TaskName 'Update AzureAD PS Module' @TaskParams -Trigger $triggers
Start-Sleep -Seconds 3
#endregion
